...
DOCUMENT_METADATA = re.compile(r"công ty|trung tâm điều độ|số phiếu|trang\s*:|phiếu chỉnh định", re.IGNORECASE)
...
def _is_document_metadata(row: dict[str, Any]) -> bool:
    """Loại header phiếu bằng nội dung metadata chung, không theo tọa độ mẫu."""

    return bool(DOCUMENT_METADATA.search(" ".join(_text(block) for block in row["blocks"])))
...
def reconstruct_page(
    ...
    fallback_content = [row for row in fallback_rows if not _is_document_metadata(row)]
